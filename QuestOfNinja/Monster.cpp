...
int Monster::updateAttack()
{
	return this->row;
}

void Monster::updateDamage(int damage)
{
	this->fadeDamage = 255;
...
void Monster::Update(float deltaTime, sf::Vector2f pos, int hit, bool playerdie)
{
	this->velocity.x = 0;
	...
	if (!this->die)
	{
		...
		else if (abs(this->body.getPosition().y - pos.y) < 250 and abs(this->body.getPosition().x - pos.x) < 500 and abs(this->body.getPosition().x - pos.x) > this->dis and !playerdie)
		{
			this->row = 2;
		...
		else if (abs(this->body.getPosition().x - pos.x) < this->dis + 10 and abs(this->body.getPosition().y - pos.y) < 50 and this->totaltimeattack > 0.5f and !playerdie)
		{
			this->row = 3;
			if (this->body.getPosition().x > pos.x)
			{
...
float Monster::updateHp(float hit)
{
	this->HP -= hit;
	if (this->HP <= 0)
		this->HP = 0;
